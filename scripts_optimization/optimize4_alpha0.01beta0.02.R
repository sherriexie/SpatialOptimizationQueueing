...
solns <- read_rds("solns/asaT16_a0.01_b0.02_round3.rds")
...
##-----------------------------------------------------------------------------
# 2a. Create 500 offspring sets (to serve as starting sets) by mating two 
#     optimized parents
##-----------------------------------------------------------------------------
...
for(i in 1:500){
  randomsupplysets[[i]] <- RandomSelection(zones = zones, minsites = 4, k = 20, 
                                           totalsites = 70)
}
...
for (i in 1:n_iterations){  
  ...
  solutions[[i]] <- RunOptimizerSelectedPoints(probmatrix, 
                                               startsupply = offspring[[i]], 
...
saveRDS(soln_out, file = "asaT16_a0.01_b0.02_round4.rds")