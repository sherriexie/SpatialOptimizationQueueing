...
solutions[[i]] <- RunOptimizerNaive(probmatrix, k = 20, mu = 0.5, t = 960, 
                               habitability = 0.57, hh_withdogs = 0.4)

# Format solutions and save
soln_out <- FormatSolutions(solutions)
saveRDS(soln_out, file = "asaT16_queuenaive_firstrun.rds")
...
# Format solutions and save
soln_out <- FormatSolutions(solutions)
saveRDS(soln_out, file = "asaT16_queuenaive_1to200.rds")

# Runs 201-1000
tic()
for (i in 201:1000){  # Run algorithm over iterations
  print(paste0("============ Iteration ", i, " of ", n_iterations, " ============"))
  set.seed(rand_seed[i])
  solutions[[i]] <- RunOptimizerNaive(probmatrix, k = 20, mu = 0.5, t = 960, 
                                      habitability = 0.57, hh_withdogs = 0.4)
...
# Format solutions and save
soln_out <- FormatSolutions(solutions)
saveRDS(soln_out, file = "asaT16_queuenaive_round1.rds")